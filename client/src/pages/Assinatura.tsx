...
import {
  CheckCircle2, MessageCircle, LogOut, CreditCard, Zap, Loader2,
  ChevronLeft, ChevronRight, History, ShieldCheck, XCircle,
} from "lucide-react";
...
function statusBadge(status: string | null) {
  ...
  const color = isGood ? "bg-green-500/15 text-green-400" : isBad ? "bg-destructive/15 text-destructive" : "bg-amber-500/15 text-amber-400";
  return <span className={`text-[10px] px-2 py-0.5 rounded-full font-medium ${color}`}>{status}</span>;
}

function SubscribeDialog({ plan, isPlanChange, onClose }: { plan: PaidPlanId; isPlanChange: boolean; onClose: () => void }) {
  ...
  const subscribe = trpc.billing.subscribe.useMutation({
    onSuccess: (data) => {
      if (data.checkoutUrl) {
        toast.success("Redirecionando pro checkout...");
        window.location.href = data.checkoutUrl;
        return;
      }
      toast.success("Plano atualizado! O novo valor já vale a partir da próxima cobrança.");
  ...
  return (
    <Dialog open onOpenChange={(open) => !open && onClose()}>
      <DialogContent>
        <DialogHeader>
          <DialogTitle>{isPlanChange ? "Trocar para o plano" : "Assinar plano"} {PLAN_CONFIG[plan].name}</DialogTitle>
        </DialogHeader>
        <div className="space-y-3">
          <p className="text-xs text-muted-foreground">
            {isPlanChange
              ? "Esses dados confirmam a troca de plano — o novo valor é aplicado na sua assinatura já existente, sem gerar uma cobrança nova agora."
              : "Esses dados são pra emissão da cobrança. O pagamento em si (cartão, PIX ou boleto) é feito na próxima tela, direto com o ASAAS — nenhum dado de cartão passa pelo nosso servidor."}
          </p>
          <div>
          ...
          <div>
            ...
            <Input
              ...
              inputMode="numeric"
            />
            {touched.cpfCnpj && cpfCnpjError && <p className="text-[10px] text-destructive mt-1">{cpfCnpjError}</p>}
          </div>
          <div>
            ...
            />
            {touched.email && emailError && <p className="text-[10px] text-destructive mt-1">{emailError}</p>}
          </div>
          <div>
            ...
            <Input
              ...
              onBlur={() => setTouched((t) => ({ ...t, mobilePhone: true }))}
              placeholder="(00) 00000-0000"
              inputMode="numeric"
            />
            {touched.mobilePhone && phoneError && <p className="text-[10px] text-destructive mt-1">{phoneError}</p>}
          </div>
        </div>
        <DialogFooter>
          <Button variant="ghost" onClick={onClose} disabled={subscribe.isPending}>Cancelar</Button>
          <Button onClick={handleSubmit} disabled={subscribe.isPending} className="racing-gradient text-white">
            {subscribe.isPending
              ? <><Loader2 className="w-4 h-4 mr-1.5 animate-spin" /> {isPlanChange ? "Trocando plano..." : "Gerando cobrança..."}</>
              : isPlanChange ? "Confirmar troca de plano" : "Ir para pagamento"}
          </Button>
        </DialogFooter>
      </DialogContent>
    </Dialog>
...
export default function Assinatura() {
  ...
  const { data: myPlan } = trpc.billing.getMyPlan.useQuery();
  const { data: history } = trpc.billing.getMyPaymentHistory.useQuery({ limit: PAGE_SIZE, offset: page * PAGE_SIZE });
    ...
    <div className="min-h-screen bg-background py-10 px-4">
      <div className="max-w-4xl mx-auto">
        <div className="text-center mb-8">
          ...
          <h1 className="font-heading text-2xl font-bold text-foreground mb-2">Sua assinatura</h1>
          {isPaidActive ? (
            <p className="text-sm text-green-400 flex items-center justify-center gap-1.5">
              <ShieldCheck className="w-4 h-4" /> Plano {PLAN_CONFIG[myPlan!.plan as PaidPlanId]?.name || myPlan!.plan} ativo
            </p>
          ) : isSuspended ? (
            <p className="text-sm text-destructive flex items-center justify-center gap-1.5">
              <XCircle className="w-4 h-4" /> Assinatura em atraso — regularize o pagamento pra reativar o acesso
            </p>
          ) : (
            <p className="text-sm text-muted-foreground max-w-md mx-auto">
              Escolha um plano abaixo pra continuar usando o {brandName} sem interrupção.
            </p>
          )}
        </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-4 mb-10">
          {(Object.values(PLAN_CONFIG)).map((plan) => {
            ...
            return (
              <div key={plan.id} className={`racing-card p-6 flex flex-col ${plan.highlight ? "border-primary/60 ring-1 ring-primary/40" : ""} ${isCurrentPlan ? "ring-2 ring-green-500/60" : ""}`}>
                ...
                <div className="mb-1 flex items-baseline gap-2 flex-wrap">
                  <span className="text-sm text-muted-foreground line-through">{formatCentsToBRL(plan.originalPriceCents)}</span>
                  <span className="text-2xl font-bold text-foreground">{formatCentsToBRL(plan.monthlyPriceCents)}</span>
                </div>
                ...
                {isCurrentPlan ? (
                  <Button variant="outline" className="w-full text-destructive hover:text-destructive"
                    onClick={() => { if (confirm("Cancelar sua assinatura?")) cancelMut.mutate(); }}
                  ...
                  <div className="space-y-2">
                    ...
                    <Button variant="ghost" asChild className="w-full text-xs">
                      <a href={WHATSAPP_CONTACT} target="_blank" rel="noopener noreferrer">
                        <MessageCircle className="w-3.5 h-3.5 mr-1.5" /> Falar com vendas
                      </a>
                    </Button>
                  </div>
                )}
        ...
        <div className="racing-card p-6 mb-10">
                ...
                <TableBody>
                  {history.items.map((item) => (
                    <TableRow key={item.id}>
                      ...
                      <TableCell className="text-xs">{item.value ? formatCentsToBRL(Math.round(Number(item.value) * 100)) : "—"}</TableCell>
                      <TableCell className="text-xs text-muted-foreground">{item.dueDate ? new Date(item.dueDate).toLocaleDateString("pt-BR") : "—"}</TableCell>
                      <TableCell className="text-xs text-muted-foreground">{new Date(item.createdAt).toLocaleDateString("pt-BR")}</TableCell>
                    </TableRow>
                  ))}
                </TableBody>
              </Table>
        ...
        <div className="text-center">
          ...
          >
            <LogOut className="w-4 h-4 mr-1.5" /> Sair de {brandName}
          </Button>
        </div>
      </div>

      {subscribingPlan && (
        <SubscribeDialog plan={subscribingPlan} isPlanChange={!!isPaidActive} onClose={() => setSubscribingPlan(null)} />
      )}
    </div>
  );
}
