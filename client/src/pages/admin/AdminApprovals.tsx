...
import { Check, X, Car, Clock, AlertCircle, Loader2, Home } from "lucide-react";
    ...
    <DashboardLayout>
      <div className="space-y-6">
        <div className="flex items-center justify-between">
          <div>
            <h1 className="font-heading font-bold text-2xl text-foreground">Aprovar Vendas</h1>
            <p className="text-muted-foreground text-sm mt-1">Vendas registradas pelos vendedores aguardando sua aprovação</p>
          </div>
        ...
        {isLoading ? (
          ...
          <Card className="border-border bg-card">
            <CardContent className="py-12 text-center">
              ...
              <p className="text-muted-foreground mb-6">Não há vendas pendentes de aprovação no momento.</p>
              <Button
                onClick={() => setLocation("/")}
                variant="outline"
                className="gap-2"
          ...
          <div className="space-y-3">
            {pendingSales.map(sale => {
              ...
              return (
                <Card key={sale.id} className="bg-card border-yellow-500/20 hover:border-yellow-500/40 transition-colors">
                  <CardContent className="p-4">
                    <div className="flex items-start gap-4">
                      ...
                      <div className="flex-1 min-w-0">
                        ...
                        {competition && (
                          <p className="text-muted-foreground text-xs mt-1">Competição: {competition.name}</p>
                        )}
