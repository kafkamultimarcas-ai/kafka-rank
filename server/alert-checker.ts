 ...
 *  NOTE: Follow-up WhatsApp messages are DISABLED.
 *  They were causing mass spam because the in-memory Set resets on server restart.
...
async function _checkAlertsInner() {
  try {
    ...
    // 3. Appointment follow-up via WhatsApp is DISABLED
    // Was causing spam - will be replaced by configurable inactive dispatch with DB tracking
  } catch (err) {
    console.error("[Alert Checker] Error:", err);
  }
}
...
export function startAlertChecker(intervalMinutes: number = 2) {
  ...
  console.log(`[Alert Checker] Scheduled every ${intervalMinutes} minutes (SDR: ${SDR_THRESHOLD_MINUTES}min, Vendedor: ${SELLER_THRESHOLD_MINUTES}min). Follow-up WhatsApp DISABLED.`);
}
